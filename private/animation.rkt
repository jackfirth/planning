...
(module private racket/base
  ...
  (define convertible<%>
    (interface*
     ()
     ([prop:convertible
       (λ (v request default) (send v convert request default))])
     convert))
  ...
  (define animation-snip%
    (class* snip% (convertible<%>)
      ...
      (define/public-final (advance-frame)
      ...
      (define/override-final (draw dc x y . unused)
      ...
      (define (convert-to-gif-bytes)
        ;; The GIF file format defines the "dimension" type as an integer
        ;; between 0 and 65535 inclusive, and this type is used for various
        ;; encoded fields in control messages. One of these fields is the amount
        ;; of time (in centiseconds) between images. As a result, extremely slow
        ;; animations are impossible to render as GIFs without inserting
        ;; duplicate frames to ensure the delay between frames is never greater
        ;; than 655.35 seconds. Instead of bothering with this, we just fail
        ;; fast with an error message that explains the situation.
...
frames, or choose a different file format."
        ...
        (for ([frame (in-vector frames-vector)])
          (define x (frame-top-left-point-x frame))
          (define y (frame-top-left-point-y frame))
          (gif-add-frame gifstream x y frame frame-interval-centiseconds))
        (gif-end gifstream)
  ...
  (define (gif-add-frame gifstream x y p duration-centiseconds)
    ...
    (gif-add-control gifstream 'restore-bg #f duration-centiseconds transparent)
